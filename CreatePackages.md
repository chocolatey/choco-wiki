...
* Better don’t use an icon that has a very low resolution. In the chocolatey gallery the icons will be scaled to 50&nbsp;×&nbsp;50 and 128&nbsp;×&nbsp;128 pixels. If an icon is vastly smaller than these resolutions, it will be displayed blurry and have distinctive scaling artifacts. In this case it’s better to dispense with the icon, because it would look very unsightly.
* Applications logos with lots of details that are barely visible at 50 or 128&nbsp;px width/height are not suitable as package icons. This is also the reason why a Windows ICO file often contains icons in multiple resolutions, so that each resolution can have the appropriate level of detail.
* Currently the icons that are not exactly square (image width&nbsp;=&nbsp;height) get displayed distorted in the gallery. That distortion looks ugly, especially when there is a large difference between the icon width and the height. Therefore, please use square icons or make them square using an image editing tool like *GIMP*.
* Avoid GIF graphics as package icons and use PNG instead. PNG was designed to supersede GIF, because the latter has several restrictions, such as only 256 supported colors, no alpha channel transparency and a low compression.
* A good source for package icons are the official desktop icons of the corresponding application you want to make a package of. The icons can be extracted from the app executables using tools like *Resource Hacker*. From the resulting ICO file, remember to extract the icon with at least (if available) 128&nbsp;×&nbsp;128&nbsp;px and save it as PNG file. This can be done for example with *GIMP*.
